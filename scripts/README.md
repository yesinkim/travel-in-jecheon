...
- 17개 documents × 평균 7 Q&A = ~120 Q&A pairs
- Claude API 요청: 17회 (rate limiting 적용)
- 예상 토큰 사용량: ~50K input + ~100K output = ~$1-3

## 🎨 데이터 품질 관리

### Distractor 선택 전략

- **Hard distractors (30%)**: 같은 카테고리 내 다른 문서
  - 예: "의림지" 질문 → "청풍호반 케이블카" distractor (둘 다 tourism)
- **Easy distractors (70%)**: 완전히 다른 카테고리 문서
